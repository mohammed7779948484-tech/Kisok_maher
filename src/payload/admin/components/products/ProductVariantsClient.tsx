...
export function ProductVariantsClient({ data }: { data: ProductVariantsViewDTO }): React.ReactElement {
  ...
  return (
    <main className="dragon-admin relative">
      ...
      <VariantEditorDrawer canReadPrice={data.canReadPrice} media={data.media} onClose={() => setOpen(false)} onSaved={refresh} open={open} productID={data.productID} productName={data.productName} variant={editing} />
    </main>
  )
}
